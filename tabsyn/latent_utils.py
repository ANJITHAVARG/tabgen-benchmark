...
import pandas as pd
import torch
from tabsyn.vae.utils_train import preprocess
...
def recover_data(syn_num, syn_cat, syn_target, info):
    ...
    idx_mapping = info.get('idx_mapping', None)
    syn_df = pd.DataFrame()

    if idx_mapping is not None:
        idx_mapping = {int(key): value for key, value in idx_mapping.items()}
        col_indices = sorted(idx_mapping.keys())
    else:
        # Use raw column indices when idx_mapping is not present
        col_indices = list(range(len(num_col_idx) + len(cat_col_idx) + len(target_col_idx)))

    if info['task_type'] == 'regression':
        for i in col_indices:
            if i in num_col_idx:
                syn_df[i] = syn_num[:, num_col_idx.index(i)]
            elif i in cat_col_idx:
                syn_df[i] = syn_cat[:, cat_col_idx.index(i)]
            else:
                syn_df[i] = syn_target[:, target_col_idx.index(i)]
    else:
        for i in col_indices:
            if i in num_col_idx:
                syn_df[i] = syn_num[:, num_col_idx.index(i)]
